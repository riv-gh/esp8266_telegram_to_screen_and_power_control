  ...
  if (millis() - lastCheck > 5000) {
    ...
    while (numNew) {
      for (int i = 0; i < numNew; i++) {
        ...
        String text = translit(bot.messages[i].text);
        String username = translit(bot.messages[i].from_name);

        bot.sendMessage(chat_id, "Recive: " + text, "");

        display.clearDisplay();
        display.setCursor(0, 0);

        if (username.length() > 0) display.print(username);
        else display.print("<unknown>");

        display.println(":");

        if (text.length() > 0) display.println(text);
